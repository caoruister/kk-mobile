  ...
  if (!props.field.hideLabel) {
    return (
      <div className={props.className}>
        <FieldRequired field={props.field} />
        {props.field.label}:
      </div>
    );
  }
  ...
  return (
    <div className="weui-cell weui-cell_input">
      ...
      <div className="weui-cell__bd">
        <FieldReadOnly field={props.field}>{props.children}</FieldReadOnly>
      </div>
    </div>
  );
}
...
function FieldLayout(props) {
  if (!props.field.column) {
    return <FieldColumn field={props.field}>{props.children}</FieldColumn>;
  } else {
    return <FieldRow field={props.field}>{props.children}</FieldRow>;
  }
}
